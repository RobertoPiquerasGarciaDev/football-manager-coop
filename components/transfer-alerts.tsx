...
import { useGame, type TransferAlertView } from "@/lib/game-provider"
...
export function TransferAlerts() {
  const { getTransferAlertsForDashboard } = useGame()
  const alerts = getTransferAlertsForDashboard()

  return (
    <div className="mx-4 mt-3" aria-label="Transfer alerts">
      <div className="p-4 rounded-2xl bg-card border border-border/50">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[var(--alert-red-glow)] flex items-center justify-center">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--alert-red)]/15 border border-[var(--alert-red)]/20">
            <div className="w-1.5 h-1.5 rounded-full bg-[var(--alert-red)] animate-pulse-dot" />
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2.5 shrink-0">
                  <div className={`px-2 py-1 rounded-lg text-xs font-black ${getRatingColor(alert.rating)}`}>
                    {alert.rating}
                  </div>
