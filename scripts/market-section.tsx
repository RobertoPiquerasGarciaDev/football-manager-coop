...
export function MarketSection() {
  ...
  return (
    <DIVELEMENT className="flex flex-col gap-3 px-4 pt-4 pb-4">
      <DIVELEMENT className="grid grid-cols-2 gap-2">
        <DIVELEMENT className="flex flex-col gap-0.5 p-3 rounded-xl bg-card border border-border/50">
      ...
      <DIVELEMENT className="flex items-center p-1 rounded-xl bg-card border border-border/50">
        {([
      ...
      {(tab === "browse" || tab === "watchlist") && (
        <>
          <DIVELEMENT className="flex flex-col gap-2">
            ...
            <DIVELEMENT className="flex items-center gap-1.5 overflow-x-auto pb-0.5">
              {posFilters.map((f) => (
                <button
                  key={f}
          ...
          <DIVELEMENT className="flex flex-col gap-1.5">
            {filtered.map(({ player, club, listing, contract }) => {
              ...
              return (
                ...
                >
                  <DIVELEMENT className="relative shrink-0">
                    <DIVELEMENT className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center">
                    ...
                    {isFavorite && (
                      <DIVELEMENT className="absolute -top-1 -left-1 w-5 h-5 rounded-md bg-[var(--amber)] flex items-center justify-center">
                  ...
                  <DIVELEMENT className="flex items-center gap-2 shrink-0">
      ...
      {tab === "bids" && (
        <DIVELEMENT className="flex flex-col gap-1.5">
          {yourBids.map((bid) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border/50 hover:bg-secondary/50 active:scale-[0.99] transition-all text-left w-full group"
              >
                <DIVELEMENT className="w-10 h-10 rounded-xl bg-[var(--amber)]/10 flex items-center justify-center shrink-0">
          ...
          {yourBids.length === 0 && (
            <DIVELEMENT className="flex flex-col items-center justify-center py-12 text-center">
